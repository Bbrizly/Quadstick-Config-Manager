...
function object(value, where) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) die(`${where}: expected object`);
  return value;
}
function exactKeys(value, allowed, where) {
  for (const key of Object.keys(value)) if (!allowed.has(key)) die(`${where}: unknown field ${key}`);
}
function string(value, where, max = 120) {
  if (typeof value !== 'string' || value.length === 0 || value.length > max) die(`${where}: invalid string`);
  return value;
}
...
function readJson(file) {
  let parsed;
  try { parsed = JSON.parse(fs.readFileSync(file, 'utf8')); } catch (e) { die(`${path.relative(root, file)}: ${e.message}`); }
  return parsed;
}
...
function assertFileId(file, value) {
  const base = path.basename(file, '.json');
  if (base !== value.id) die(`${path.relative(root, file)}: file name must equal id (${value.id})`);
}

export function validateGame(value, where = 'game') {
  object(value, where);
  exactKeys(value, new Set(['schemaVersion', 'id', 'name', 'status', 'platforms', 'actions']), where);
  ...
  unique(value.platforms, `${where}.platforms`);
  for (const p of value.platforms) if (!platforms.has(p)) die(`${where}.platforms: unsupported ${p}`);
  ...
  for (const [i, action] of value.actions.entries()) {
    const at = `${where}.actions[${i}]`; object(action, at);
    exactKeys(action, new Set(['id', 'label', 'category', 'defaultOutputs']), at);
    ...
    if (action.defaultOutputs !== undefined) {
      object(action.defaultOutputs, `${at}.defaultOutputs`);
      exactKeys(action.defaultOutputs, platforms, `${at}.defaultOutputs`);
      for (const [platform, outputs] of Object.entries(action.defaultOutputs)) {
        ...
        for (const output of outputs) {
          string(output, `${at}.defaultOutputs.${platform}`, 60);
          if (!/^[a-z0-9]+(?:[-_][a-z0-9]+)*$/.test(output)) die(`${at}.defaultOutputs.${platform}: invalid output token`);
        }
      }
    }
  }
  unique(actionIds, `${where}.actions`);
  return value;
}

export function validateDevice(value, where = 'device') {
  object(value, where);
  exactKeys(value, new Set(['schemaVersion', 'id', 'name', 'manufacturer', 'status', 'inputs']), where);
  if (value.schemaVersion !== 1) die(`${where}: schemaVersion must be 1`);
  ...
  for (const [i, input] of value.inputs.entries()) {
    const at = `${where}.inputs[${i}]`; object(input, at);
    exactKeys(input, new Set(['id', 'label', 'kind']), at);
    id(input.id, `${at}.id`); ids.push(input.id); string(input.label, `${at}.label`, 80);
    if (!inputKinds.has(input.kind)) die(`${at}.kind: invalid input kind`);
  }
  unique(ids, `${where}.inputs`);
  return value;
}

export function validateProfile(value, games, devices, where = 'profile') {
  object(value, where);
  exactKeys(value, new Set(['schemaVersion', 'id', 'gameId', 'platform', 'deviceId', 'variant', 'contributor', 'source', 'bindings', 'tags']), where);
  if (value.schemaVersion !== 1) die(`${where}: schemaVersion must be 1`);
  id(value.id, `${where}.id`); id(value.gameId, `${where}.gameId`); id(value.deviceId, `${where}.deviceId`);
  ...
  const actionSet = new Set(game.actions.map(x => x.id));
  const inputSet = new Set(device.inputs.map(x => x.id));
  const boundActions = [];
  for (const [i, binding] of value.bindings.entries()) {
    const at = `${where}.bindings[${i}]`; object(binding, at);
    exactKeys(binding, new Set(['actionId', 'inputId', 'behavior']), at);
  ...
  unique(value.tags, `${where}.tags`);
  for (const tag of value.tags) if (!tags.has(tag)) die(`${where}.tags: unsupported ${tag}`);
  const expectedPrefix = `${value.gameId}--${value.platform}--${value.deviceId}--${value.variant}--`;
  if (!value.id.startsWith(expectedPrefix)) die(`${where}.id: must start with ${expectedPrefix}`);
  return value;
}

function loadSet(dir, validator) {
  const map = new Map();
  for (const file of jsonFiles(dir)) {
    const value = validator(readJson(file), path.relative(root, file));
    assertFileId(file, value);
    if (map.has(value.id)) die(`${path.relative(root, file)}: duplicate id ${value.id}`);
...
function buildIndex(games, devices, profiles) {
  return {
    schemaVersion: 1,
    games: [...games.values()].filter(x => x.status === 'published').sort((a, b) => a.id.localeCompare(b.id)),
    devices: [...devices.values()].filter(x => x.status === 'published').sort((a, b) => a.id.localeCompare(b.id)),
...
function main() {
  ...
  if (!['--check', '--write-index'].includes(mode)) die('usage: validate.mjs [--check|--write-index]');
  const games = loadSet(path.join(registry, 'games'), validateGame);
  const devices = loadSet(path.join(registry, 'devices'), validateDevice);
  const profiles = loadSet(path.join(registry, 'profiles'), value => validateProfile(value, games, devices));

  const fixtureGames = loadSet(path.join(registry, 'fixtures', 'games'), validateGame);
  const fixtureProfiles = loadSet(path.join(registry, 'fixtures', 'profiles'), value => validateProfile(value, new Map([...games, ...fixtureGames]), devices));
  void fixtureProfiles;

  const expected = stable(buildIndex(games, devices, profiles));
  const indexFile = path.join(registry, 'index.json');
  if (mode === '--write-index') fs.writeFileSync(indexFile, expected);
  else {
    const actual = fs.existsSync(indexFile) ? fs.readFileSync(indexFile, 'utf8') : '';
    if (actual !== expected) die('registry/index.json is stale; run node tools/registry/validate.mjs --write-index');
  }
