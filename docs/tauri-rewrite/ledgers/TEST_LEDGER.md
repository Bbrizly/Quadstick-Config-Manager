...
T-006 is IMPLEMENTED, not PARITY-TESTED. It is a unit test written from the
shipped `Device.cs`, not an artifact generated by the frozen C# oracle the way
`qcm-config` parity works. It moves to PARITY-TESTED when the same sequence runs
against a QuadStick.
