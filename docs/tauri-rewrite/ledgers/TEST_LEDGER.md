...
| T-006 | discovery stale/multiple | `qcm-core/tests/device_discovery.rs` on the fake | real multiple/remount | IMPLEMENTED |
| T-007 | install transaction faults | `qcm-core/tests/device_install.rs` every stage | unplug/full/read-only on a stick | IMPLEMENTED |
| T-008 | library/delete/order | `qcm-core/tests/device_library.rs` | real device | IMPLEMENTED |
...
T-006, T-007 and T-008 are IMPLEMENTED, not PARITY-TESTED. They are unit tests
written from the shipped `Device.cs` and its three test files, not artifacts
generated by the frozen C# oracle the way `qcm-config` parity works. They move to
PARITY-TESTED when the same sequences run against a QuadStick.
