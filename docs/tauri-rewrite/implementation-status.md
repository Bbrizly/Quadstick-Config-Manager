...
Nothing in Phase 3 exists yet. There is no `qcm-core` session manager, no device port, no discovery, no install transaction, no HID, no Tauri project and no frontend. `crates/` holds one crate.
