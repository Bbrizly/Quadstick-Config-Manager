...
| TASK-014 serializer + normalization | **NEXT** | exact normalized bytes, idempotence, and comment-column retention will be differential-gated |

## Current verified checkpoint

TASK-013 completed at `9f0a284f659d9a75b034c5a57b61504dd9570e15`. Rewrite parity run `33277932680` passed the full legacy .NET suite, oracle generation and schema validation, Rustfmt, Clippy with warnings denied, and the complete Rust differential suite including ordered validation issue parity.

Validation now preserves the frozen implementation's firmware-derived severity and location behavior for filename/device limits, mode and binding limits, channels, output/function/input vocabulary, Preferences sheets, mode preference overrides, firmware-specific warnings, and threshold ordering. Parser-originated issues remain ordered before validator issues.

The parser preserves the frozen implementation's important structural distinctions: raw firmware keyword acceptance versus loose display discovery, true blank lines versus comma rows, missing-separator recovery, one-based sheet/binding rows, C..J input gaps, and the column-L action-name extension. K and M+ remain outside the parsed device projection.
