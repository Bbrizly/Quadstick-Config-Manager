...
`qcm-core/src/devices/` holds one service that owns the storage port, the
off-device backup area and the confirmation ledger: discovery with the shipped 3
second scan cache, the install transaction, and the library operations.

There is still no storage adapter, no HID and no frontend beyond the scaffold.
