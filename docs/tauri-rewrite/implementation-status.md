...
Phase 3 has started at the contract end. `crates/` now holds three crates: `qcm-config`, `qcm-core` and `qcm-testkit`. `qcm-core` carries the error families, operation identity, expiring confirmations and the device storage port. `qcm-testkit` carries a fake QuadStick drive that can fail at any stage of the install sequence.

None of it is wired to anything yet. There is still no session manager, no discovery adapter, no install transaction, no library operations, no HID, no Tauri project and no frontend. The port is a trait with one implementation, and that implementation is a fake.
...
TASK-021 and TASK-022 added no external crate at all. `qcm-core` depends on `qcm-config` and `serde`, with `serde_json` for tests; `qcm-testkit` depends on `qcm-core` alone. Neither has Tauri, an OS crate, a network client or anything that writes a file: the storage port is a trait, the adapter that touches a real volume arrives in TASK-023.
